...
enum {
  DEBUG_LASTOP_WALLOCREARRANGE,
  DEBUG_LASTOP_WALLOCALLOC,
  DEBUG_LASTOP_FREE,
  DEBUG_LASTOP_COALESCE,
  DEBUG_LASTOP_COALESCE_SINGLEPICK,
  DEBUG_LASTOP_WALLOCSINGLEPICK,
  ...
  if(n <= 0) {
    ...
    if(zone.first != NULL && zone.last != NULL) {
      if(zone.first == zone.last) {
        zone.first->lastop = DEBUG_LASTOP_WALLOCSINGLEPICK;
        zone.first->isfree = 0;
...
void *wcalloc(int nmemb, int size) {
  if(size >= 1 && nmemb >= 1) {
    ...
    void *ptr_toret = walloc(size_bytes);
    if(ptr_toret == NULL) {
      return NULL;
    }
